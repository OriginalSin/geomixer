...
var getParams = function(prop, dm, gmx) {
	...
	if (dm && dm.options.Temporal && (prop.UseTiles === false || gmx.skipTiles === 'NotVisible' || gmx.needBbox)) {
		var maxDateInterval = dm.getMaxDateInterval(),
			curr = new Date(),
			beginDate = maxDateInterval.beginDate || gmx.beginDate || curr,
			endDate = maxDateInterval.endDate || gmx.endDate || curr;
