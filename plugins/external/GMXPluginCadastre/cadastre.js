(function () {
    ...
    var layerGroup,
		searchControl,
		inputShowObject,
		proxy = 'http://pkk5.kosmosnimki.ru/',	// http://kosmosnimki.ru/pkk5/ http://pkk5.rosreestr.ru/
		exportIcon,
		lastFeature,
		lastFeatureId,
		lastOverlays = [],
	...
	*/
    ...
    L.CadUtils = {
        ...
        getFeature: function(id, cadCount, featureCont, layer, map) {
			lastFeature = null;
			L.gmxUtil.requestJSONP(proxy +  'api/features/' + layer.id + '/' + id, {},
				{
				...
				map.fire('cadastreFeature', {cadastreFeature: lastFeature});
			});
        },
        ...
        balloon: function(ev) {
            if (ev.type === 'click' && this._map) {
                ...
                L.gmxUtil.getCadastreFeatures(L.extend(ev, {url: proxy + 'api/features/', callbackParamName: 'callback'})).then(function(data) {
        ...
        afterViewer: function (params, map) {
			...
			if (!layerGroup) {
				...
				var clickOn = function(ev) {
						if (!map._skipClick && !map.isGmxDrawing()) {
							map._skipClick = true;
					...
					searchHook = function(str) {
						...
						if (!it) { return false; }
						L.gmxUtil.requestJSONP(proxy +  'api/features/' + it.id,
							{
								WrapStyle: 'func',
								text: str,
								limit: it.limit || 11,
